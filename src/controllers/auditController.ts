...
class AuditController {
  ...
  private async calculateEnergyEfficiencyScore(
    ...
    try {
      ...
      SELECT 
        AVG(CASE 
          WHEN cc.section_code LIKE '%energy%' OR cc.section_code LIKE '%efficiency%' 
  ...
  private async calculatePowerQualityScore(
    ...
    try {
      ...
      SELECT 
        AVG(CASE 
          WHEN cc.section_code LIKE '%power%' OR cc.section_code LIKE '%quality%' OR cc.section_code LIKE '%voltage%'
          THEN CASE WHEN cc.status = 'compliant' THEN 100 ELSE 0 END 
  ...
  private async calculateSafetyComplianceRate(
    ...
    try {
      ...
      SELECT 
        AVG(CASE 
          WHEN cc.section_code LIKE '%safety%' OR cc.section_code LIKE '%OSHS%' OR cc.severity = 'critical'
          THEN CASE WHEN cc.status = 'compliant' THEN 100 ELSE 0 END 
  ...
  /**
   * Identify priority areas based on compliance gaps - supports research recommendations
   */
  private async identifyPriorityAreas(
    ...
    try {
      ...
      SELECT 
        ...
        COUNT(CASE WHEN cc.severity = 'critical' AND cc.status = 'non_compliant' THEN 1 END) as critical_violations,
        (COUNT(CASE WHEN cc.status = 'non_compliant' THEN 1 END) / COUNT(*)) * 100 as violation_rate
      FROM audits a
      JOIN compliance_checks cc ON a.id = cc.audit_id
      ${dateWhereClause}
